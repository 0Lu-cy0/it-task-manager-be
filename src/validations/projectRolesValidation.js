...
const BASE_ROLE_SCHEMA = Joi.object({
  name: Joi.string().required().min(3).max(50).trim().pattern(/^[a-zA-Z0-9\s-_]+$/).messages({
...
const validateUpdateRolePermissions = async (data) => {
  ...
  if (!permissionDocs.length) {
    throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, 'Không tìm thấy quyền hợp lệ trong hệ thống')
  }
  const validPermissionIds = permissionDocs.map(p => p._id.toString())

  const schema = Joi.object({
    permissions: Joi.array()
      .items(Joi.string().valid(...validPermissionIds).messages({
